...
function locationIpAndCreate(carParkArray) {
    ...
    geolocation.getCurrentPosition(function (position) {
        if(this.getStatus() === BMAP_STATUS_SUCCESS){             //通过Geolocation类的getStatus（）可以判断是否已经成功定位
            ...
            addClickListener(longitude,latitude,carParkArray);
        }else {
...
function addMapOverlay(carParkArray){
    ...
    for (let i = 0; i < carParkArray.length; i++) {
        if(carParkArray[i].type == 0){
            let carPark = {
                content:"剩余："+carParkArray[i].rest+"个车位",
...
function deletePoint() {
    var allOverlay = map.getOverlays();   //获得所有的自定义标注
    for (var i = 0; i < allOverlay.length - 1; i++) {
        map.removeOverlay(allOverlay[i]);
    }
}
//经度纬度增加至input中，并新增个标注
function addClickListener(longitude,latitude,carParkArray){
    map.addEventListener("click",function (e) {
        longitude.value = e.point.lng; //经度的input框的value
        latitude.value = e.point.lat; //纬度的input框的value
        var marker = new BMap.Marker(e.point);
        map.addOverlay(marker);
        map.panTo(e.point);
        deletePoint();  //删除所有标注
        addMapOverlay(carParkArray);   //再把需要的标注添加进去
    });
}
//设置地图事件
